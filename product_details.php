    ...
    <div class="product-detail">
        ...
        <div class="product-detail-left">
            ...
            <div style="position:relative; width:100%; max-width:500px; margin:20px auto 10px auto;">
                <img id="main-image" src="/assets/img/<?php echo htmlspecialchars($product['image']); ?>"
                    alt="<?php echo htmlspecialchars($product['name']); ?>"
                    style="width:100%; max-width:500px; height:500px; object-fit:cover; display:block; border-radius:12px;">

                <span style="position:absolute; right:0; top:0; background:rgba(183,94,31,0.9); color:#fff;
                         padding:8px 16px; border-top-right-radius:12px; font-size:20px; font-weight:bold;">
            ...
            <div class="thumbnail-gallery"
                style="display:flex; justify-content:flex-start; gap:12px; max-width:500px; margin:15px auto 30px auto; overflow-x:auto;">
                <?php foreach ($gallery_images as $index => $img_name): ?>
                    <img src="/assets/img/<?php echo htmlspecialchars($img_name); ?>" class="thumb-item"
                        onclick="changeMainImage(this, '/assets/img/<?php echo htmlspecialchars($img_name); ?>')" style="width:55px; height:55px; margin:0 !important; display:block; flex-shrink:0;
                            object-fit:cover; border-radius:6px; cursor:pointer;
                            border:2px solid <?php echo $index === 0 ? '#9d503b' : 'transparent'; ?>;
                            transition:border-color 0.2s;" alt="Gallery Image">
                <?php endforeach; ?>
            </div>
            ...
            <div style="margin-top:40px;">
                ...
                <div style="margin-bottom:18px;">
                    ...
                    <?php for ($i = 1; $i <= 5; $i++): ?>
                        <span
                            style="color:<?php echo $i <= round($avg_rating) ? '#f7b731' : '#ddd'; ?>; font-size:1.2em;">&#9733;</span>
                    <?php endfor; ?>
                    <span style="color:#888; font-size:1em; margin-left:10px;">
                        (
                        <?php echo $total_reviews; ?> review
                        <?php echo $total_reviews == 1 ? '' : 's'; ?>)
                    </span>
                </div>
                    ...
                    <?php while ($review = $review_result->fetch_assoc()): ?>
                        <div style="border-bottom:1px solid #eee; padding:14px 0;">
                            <div>
                                <?php for ($i = 1; $i <= 5; $i++): ?>
                                    <span
                                        style="color:<?php echo $i <= $review['rating'] ? '#f7b731' : '#ddd'; ?>; font-size:1.1em;">&#9733;</span>
                                <?php endfor; ?>
        ...
        <div class="product-detail-right">
            ...
            <form method="post" id="cart-form">
                ...
                <div class="card">
                    <?php foreach ($cards as $card): ?>
                        <label data-card-id="<?php echo $card['id']; ?>"
                            ...
                            <input type="radio" name="card" value="<?php echo $card['id']; ?>"
                            ...
                            <div>
                                ...
                                <div style="color:#840000;">+
                                    <?php echo number_format($card['price']); ?> VND
                                </div>
                            </div>
                        </label>
                    <?php endforeach; ?>
                ...
                <!-- Buttons -->
                <input type="submit" name="add_to_cart" value="🛒 Add to Cart" style="width:30%; background-color:#9d503b; color:white; padding:14px 20px;
                          border:none; border-radius:4px; cursor:pointer;">

                <button type="button" id="checkout-btn" style="width:30%; background-color:#840000; color:white; padding:14px 20px;
