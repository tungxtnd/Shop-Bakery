...
<body>
    ...
    <p style="text-align:center;">We design bouquets the French way, using seasons and our Parisian roots as
        inspiration.</p>
    <form class="search-bar" method="get" action="shop.php">
        ...
        <a href="shop.php"
            style="color:#cb5d00; text-align:center; padding: 8px 12px; border: 1px solid #ccc; border-radius: 4px; text-decoration: none;">Reset</a>
    </form>
    <div class="shop-main">
        <form class="sidebar" method="get" action="shop.php">
            ...
            <input type="hidden" name="collection_id"
                value="<?php echo isset($_GET['collection_id']) ? intval($_GET['collection_id']) : ''; ?>">
            <div style="margin-bottom:12px;">
                <label>Min price:</label>
                <input type="number" step="10000" min="0" name="min_price" placeholder="Min" min="0"
                    value="<?php echo isset($_GET['min_price']) ? intval($_GET['min_price']) : ''; ?>">
            </div>
            <div style="margin-bottom:12px;">
                <label>Max price:</label>
                <input type="number" step="10000" min="0" name="max_price" placeholder="Max" min="0"
            ...
            <div style="margin-bottom:18px;">
                ...
                <select name="collection_id" id="collection_id" style="width:100%;padding:6px;border-radius:4px;">
                    ...
                    <?php foreach ($collections as $col): ?>
                        <option value="<?php echo $col['id']; ?>" <?php if (isset($_GET['collection_id']) && $_GET['collection_id'] == $col['id'])
                               echo 'selected'; ?>>
    ...
    <?php if ($total_pages > 1): ?>
        <div class="pagination" style="margin: 32px 0; display: flex; justify-content: center; gap: 8px;">
            <?php for ($p = 1; $p <= $total_pages; $p++): ?>
                ...
                ?>
                <a href="<?php echo $url; ?>" style="padding:8px 14px;border-radius:4px;<?php if ($p == $page)
                       echo 'background:#cb5d00;color:#fff;';
                   else
                       echo 'background:#fff;color:#e75480;border:1px solid #840000;'; ?>">
                    <?php echo $p; ?>
                </a>
            <?php endfor; ?>
        </div>
